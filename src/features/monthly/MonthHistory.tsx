...
export function MonthHistory() {
  ...
  return (
    <Panel title={`Past months (${rows.length})`}>
      <table className="t" aria-label="Month history">
        ...
        <tbody>
          {rows.map((m) => (
            <tr key={m.id} className="clickable" onClick={() => navigate({ page: 'monthly', monthId: m.id })}>
              <td>
                <button
                  className="btn link"
                  onClick={(e) => {
      ...
      <p className="small subtle" style={{ marginBottom: 0 }}>
        ...
        total. Their original “Balanced/Review” result reflected the journal difference only; the status here
        also applies the full reconciliation rules.
      </p>
    </Panel>
  );
}
