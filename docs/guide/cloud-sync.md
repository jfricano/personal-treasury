...
The web app asks for credentials each new browser session. Browser data can be cleared by the browser or device, so keep the cloud service's persistent volume backed up and retain occasional complete JSON backups. For now, use one private web tab at a time for this treasury.
